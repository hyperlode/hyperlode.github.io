...
function addRadioButtons(elementToAttachTo, id, name, options, selected){
	// options = list of strings (which will become the radio options.)
	// selected = one of the options.
	var radioForm = document.createElement("form");

	for (var i = 0; i < options.length; i++) {
		let option_value = options[i];
		var radioButton = document.createElement("input");
		radioButton.setAttribute("type", "radio");
		radioButton.id = id + "_" + option_value;
		radioButton.name = name;
		radioButton.value = option_value;
		...
		addText(radioForm, option_value);
		// addBr(radioForm);
	}
	
	elementToAttachTo.appendChild(radioForm);
...
function addStatusLight(elementToAttachTo, colour, size){
	// this.circle_green = '<svg width="20" height="20"><circle cx="10" cy="10" r="8" stroke="black" stroke-width="1" fill="green" /></svg>';
	// this.circle_red = '<svg width="20" height="20"><circle cx="10" cy="10" r="8" stroke="black" stroke-width="1" fill="red" /></svg>';
	// colour should be a normal html colour (like red, green, blue, orange, ...)

	// let infill = '<svg width="20" height="20"><circle cx="10" cy="10" r="8" stroke="black" stroke-width="1" fill="' + colour + '" /></svg>';

	// let divStatusLight = addDiv(document.getElementById("mlink_stream_status"), "remote_radio_status_light", "remote_radio_status_light");
	// divStatusLight.innerHTML= dongle_status_light;
	...
	// shape.setAttribute("id", "nopee");
	svg.appendChild(shape);
	elementToAttachTo.appendChild(svg);
}
...
function createButtonToExecuteGeneralFunction(caption,name, id, func,arg){
	var button = document.createElement("input");
    button.type = "button";
    button.value = caption;
	button.name = name;
	...
	addFunctionToButton(button, func, arg);
	return button;
}

function addButtonToExecuteGeneralFunction(elementToAttachTo,caption,name, id, func,arg){
	var button = createButtonToExecuteGeneralFunction(caption,name, id, func,arg)
    elementToAttachTo.appendChild(button);
	return button;
}

function addFunctionToButton(buttonElement, func,arg){
	if (arg == undefined){
